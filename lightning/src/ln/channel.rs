	...
	SP::Target: SignerProvider,
{
	...
	fn new_for_outbound_channel<'a, ES: Deref, F: Deref, L: Deref>(
		...
		pubkeys: ChannelPublicKeys, _logger: L,
	...
	fn get_pending_htlc_stats(
		...
		if let Some(excess_feerate) = excess_feerate_opt {
			...
			if !self.channel_type.supports_anchors_zero_fee_htlc_tx() {
				on_counterparty_tx_dust_exposure_msat += on_counterparty_tx_accepted_nondust_htlcs
					as u64 * htlc_success_tx_weight(
					&self.channel_type,
				) * excess_feerate as u64
					/ 1000;
				on_counterparty_tx_dust_exposure_msat += on_counterparty_tx_offered_nondust_htlcs
					as u64 * htlc_timeout_tx_weight(
					&self.channel_type,
				) * excess_feerate as u64
					/ 1000;
			}
...
mod tests {
	...
	use crate::ln::channel::{
		commit_tx_fee_sat, AwaitingChannelReadyFlags, Channel, ChannelState, HTLCCandidate,
	...
	fn test_timeout_vs_success_htlc_dust_limit() {
		...
		let commitment_tx_fee_0_htlcs =
			commit_tx_fee_sat(chan.context.feerate_per_kw, 0, chan.context.get_channel_type())
				* 1000;
		let commitment_tx_fee_1_htlc =
			commit_tx_fee_sat(chan.context.feerate_per_kw, 1, chan.context.get_channel_type())
				* 1000;
